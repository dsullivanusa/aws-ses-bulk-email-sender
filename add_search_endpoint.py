#!/usr/bin/env python3
"""
Add /contacts/search endpoint to existing API Gateway
This enables DynamoDB-based name search for contacts
"""
...
def add_search_endpoint():
    """Add the /contacts/search POST endpoint to API Gateway"""
    ...
    try:
        ...
        # Check if /contacts/search already exists
        search_exists = False
        search_resource_id = None
        for resource in resources['items']:
            if resource['path'] == '/contacts/search':
                print("⚠️  /contacts/search endpoint already exists!")
                search_exists = True
                search_resource_id = resource['id']
                break
        
        # Create /contacts/search resource if it doesn't exist
        if not search_exists:
            print("Creating /contacts/search resource...")
            search_resource = apigateway.create_resource(
                restApiId=api_id,
                parentId=contacts_resource_id,
                pathPart='search'
            )
            search_resource_id = search_resource['id']
            print(f"✓ Created /contacts/search resource (ID: {search_resource_id})")
        
        # Add POST method to /contacts/search
        try:
            print("Adding POST method to /contacts/search...")
            ...
            print("⚠️  POST method already exists on /contacts/search")
        ...
        try:
            ...
            lambda_client.add_permission(
                ...
                StatementId=f'apigateway-search-{api_id}',
        ...
        deployment = apigateway.create_deployment(
            restApiId=api_id,
            stageName='prod',
            description='Added /contacts/search endpoint for DynamoDB name search'
        )
        print(f"✓ Deployment successful! (ID: {deployment['id']})")
        
        print("\n" + "="*60)
        print("✓ SUCCESS! /contacts/search endpoint has been added")
        print("="*60)
        print(f"\nAPI Endpoint: https://{api_id}.execute-api.{REGION}.amazonaws.com/prod")
        print(f"Search endpoint: https://{api_id}.execute-api.{REGION}.amazonaws.com/prod/contacts/search")
        print("\nYou can now search contacts by name directly from DynamoDB!")
        print("- No need to load all contacts first")
        print("- Works with 20,000+ contacts")
...
if __name__ == '__main__':
    print("="*60)
    print("Add /contacts/search Endpoint to API Gateway")
    print("="*60)
    print()
