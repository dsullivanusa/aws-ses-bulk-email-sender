...
def lambda_handler(event, context):
    """API Gateway Lambda handler for SES email operations"""
...
def send_ses_campaign(body, headers):
    """Send SES email campaign with attachments"""
    ...
    campaigns_table.put_item(
        Item={
            ...
            'campaign_name': body.get('campaign_name', 'Untitled SES Campaign'),
            'status': 'in_progress',
    ...
    for i, contact in enumerate(contacts):
        try:
            ...
            if success:
                ...
                update_contact_email_sent(contact['email'], body.get('campaign_name', 'Untitled'))
