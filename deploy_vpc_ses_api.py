...
def deploy_vpc_ses_api():
    """Deploy SES Lambda and private API Gateway in VPC"""
    ...
    function_name = 'vpc-ses-email-api-function'
    ...
    with zipfile.ZipFile('vpc_ses_lambda_function.zip', 'w') as zip_file:
        zip_file.write('vpc_ses_lambda_function.py', 'lambda_function.py')
    
    try:
        # Deploy Lambda function in VPC
        with open('vpc_ses_lambda_function.zip', 'rb') as zip_file:
        ...
        try:
            response = lambda_client.create_function(
                ...
                Description='VPC SES API Gateway Lambda for secure bulk email sender',
                Timeout=900,
                MemorySize=512,
                VpcConfig={
        ...
        api_response = apigateway_client.create_rest_api(
            name='vpc-ses-bulk-email-api',
            description='Private VPC SES Bulk Email Sender API',
        ...
        print(f"Private API Gateway created: {api_id}")
        ...
        lambda_client.add_permission(
            FunctionName=function_name,
            StatementId='vpc-ses-api-gateway-invoke',
        ...
        print(f"VPC SES API Gateway URL: {api_url}")
        print(f"VPC ID: {vpc_id}")
        print("API is accessible only from within the VPC")
        ...
        print(f"Error deploying VPC SES API Gateway: {str(e)}")
        return None
...
if __name__ == "__main__":
    print("Deploying VPC SES API Gateway and Lambda...")
    ...
    # result = deploy_vpc_ses_api()
    
    # if result:
    #     print("\nVPC SES API deployed successfully!")
    #     print("Access is restricted to VPC endpoints only")
    # else:
    #     print("Failed to deploy VPC SES API")